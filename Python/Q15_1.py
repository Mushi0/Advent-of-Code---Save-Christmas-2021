...
for i in range(1, m - 1):
    for j in range(1, n - 1):
        ...
        # mdl.add(x[i - 1][j] + x[i + 1][j] + x[i][j - 1] + x[i][j + 1] + \
        # x[i - 1][j - 1] + x[i - 1][j + 1] + x[i + 1][j - 1] + x[i + 1][j + 1] <= 3*x[i][j])
...
mdl.add(x[0][n - 2] + x[1][n - 1] == 2*x[0][n - 1])
mdl.add(x[m - 1][1] + x[m - 2][0] == 2*x[m - 1][0])
